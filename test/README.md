...
3. **Invited normal user before launch** — While admin has not launched, sign in as invited normal user → should see “Organisation setup in progress” on `/onboarding` with logout only.
4. **After launch** — Normal user and admin can use `/dashboard`; questionnaire and exports work; calling `/api/answers` before launch (as normal) returns 403.
